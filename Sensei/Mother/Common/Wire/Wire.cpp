...
bool TwoWire::twi_master_init(void)
{
    ...
    */
    ...
    // reserve the PPI channel
      simblee_ppi_channel_assign(PPI_channel, 0, 0);
    ...
    {
      simblee_ppi_channel_assign(PPI_channel, &twi->EVENTS_BB, &twi->TASKS_STOP);
    }
    else
    {
      simblee_ppi_channel_assign(PPI_channel, &twi->EVENTS_BB, &twi->TASKS_SUSPEND);
    }
    twi->TASKS_STARTRX = 1;
    while(true)
    {
  ...
  void TwoWire::begin(void) {
   if (twi == NRF_TWI1)
    ...
    beginOnPins(PIN_WIRE2_SCL, PIN_WIRE2_SDA);
}
...
void TwoWire::onService(void) {
  ...
  TwoWire Wire2 = TwoWire(WIRE2_INTERFACE);
