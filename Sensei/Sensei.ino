...
void SimbleeCOM_onReceive(unsigned int esn, const char *payload, int len, int rssi) {
  // Recording RSSI data
  if ((int) payload[0] >= 0 && (int) payload[0] < NETWORK_SIZE && (len == 1 || len == 8) && !MOTHER_NODE && timer.inDataCollectionPeriod(START_HOUR, START_MINUTE, END_HOUR, END_MINUTE)) {
  ...
  // Data transfer protocol
    ...
    if (transferPage == (int) payload[1] &&
        (romManager.transferredData.data[(int) payload[2]] == 0 && romManager.transferredData.data[((int) payload[2] + 1) % MAX_ROWS] == 0) &&
      ...
      romManager.transferredData.data[(int) payload[2]] = (int) payload[3] * 100000000 + (int) payload[4] * 1000000 + (int) payload[5] * 10000 + (int) payload[6] * 100 + (int) payload[7] * 1;
      romManager.transferredData.data[((int) payload[2] + 1) % MAX_ROWS] = (int) payload[8] * 100000000 + (int) payload[9] * 1000000 + (int) payload[10] * 10000 + (int) payload[11] * 100 + (int) payload[12] * 1;
...
void programSystemTime() {
  ...
  timer.setInitialTime((systemTime[0] - '0') * 10 + (systemTime[1] - '0'),
...
void synchronizeTime() {
  if (syncTime) {
    DS3231_get(&rtcTime);
    timer.setInitialTime(rtcTime.mon, rtcTime.mday, rtcTime.year_s, rtcTime.wday, rtcTime.hour, rtcTime.min, rtcTime.sec);
 ...
 */
void shareTime() {
  if (MOTHER_NODE || collectData || (!timer.timeout(&discoveryTime, SECONDS_TO_ACK_TIME * 1000) && !timer.inDataCollectionPeriod(START_HOUR, START_MINUTE, END_HOUR, END_MINUTE))) {
    DS3231_get(&rtcTime);
    if (rtcTime.sec == (lastRTCSecond + 1) % 60) {
      ...
      if ((MOTHER_NODE && timer.totalSecondsElapsed > 5) || (!MOTHER_NODE && timer.totalSecondsElapsed > 15)) {
        char payload[] = {deviceID, timer.t.month, timer.t.date, timer.t.year, timer.t.day, timer.t.hours, timer.t.minutes, timer.t.seconds};
        SimbleeCOM.send(payload, sizeof(payload));
      } else {
        char payload[] = {deviceID};
        SimbleeCOM.send(payload, sizeof(payload));
...
void acknowledgeTimeReceipt() {
  ...
  while (!MOTHER_NODE && !timer.timeout(&discoveryTime, SECONDS_TO_ACK_TIME * 1000) && !timer.inDataCollectionPeriod(START_HOUR, START_MINUTE, END_HOUR, END_MINUTE)) {
    delay(100);
    synchronizeTime();
    shareTime();
  }
  ...
  if (newData) {
    ...
    for (int i = 0; i < NETWORK_SIZE; i++) {
      ...
      if (rssiAverage > -100) {
        ...
        romManager.table.data[romManager.config.rowCounter] = data + (abs(rssiAverage % 100) * 1000000) + ((i % MAXIMUM_NETWORK_SIZE) * 100000000);
...
void writeDataRow(String data) {
  ...
  if (data == "Time") {
    timer.updateTime();
    romManager.table.data[romManager.config.rowCounter] = (timer.t.seconds % 60) + ((timer.t.minutes % 60) * 100) + ((timer.t.hours % 24) * 10000);
  } else if (data == "Accelerometer") {
    romManager.table.data[romManager.config.rowCounter] = (min(xAccelerometerDiff / (10 * accelerometerCount), 9999) * 1) + (min(zAccelerometerDiff / (10 * accelerometerCount), 9999) * 10000) + 4100000000;
  } else if (data == "Reset") {
...
void setupSensor() {
  ...
  while (!timer.isTimeSet) {
    ...
    } else if (transferROM && !MOTHER_NODE) {
